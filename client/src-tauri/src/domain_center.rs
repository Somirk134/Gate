...
use crate::runtime::{
    certificate_store_root, domain_store_path, ClientRuntimeState, UpdateTunnelRequest,
};
...
pub async fn domain_unbind_tunnel(
    state: &ClientRuntimeState,
    host: String,
) -> Result<Value, String> {
                ...
                if let Some(tunnel_id) = tunnel_id {
                    if request.action == "enable" {
                        state
                            ...
                            .await
                            .map(|_| json!({ "host": host, "ok": true, "action": "enable" }))
                    } else {
                        state
                            .stop_tunnel(tunnel_id)
                            .await
                            .map(|_| json!({ "host": host, "ok": true, "action": "disable" }))
                    }
                } else {
...
pub async fn domain_topology(
    ...
    for item in &items {
        ...
        if let Some(project_id) = project_id.filter(|value| !value.is_empty()) {
            ...
            edges.push(
                json!({ "from": format!("project:{project_id}"), "to": format!("domain:{host}") }),
            );
        }
        ...
        if let Some(tunnel_id) = tunnel_id.filter(|value| !value.is_empty()) {
            ...
            edges.push(
                json!({ "from": format!("tunnel:{tunnel_id}"), "to": format!("domain:{host}") }),
            );
        }
...
async fn collect_domain_items(
    ...
    for host in hosts {
        let tunnel = tunnels
            .iter()
            .find(|tunnel| tunnel.get("host").and_then(Value::as_str) == Some(host.as_str()));
...
fn load_certificate_index() -> HashMap<String, Value> {
    ...
    for entry in entries.flatten() {
        ...
        let expire_time = value
            ...
            .unwrap_or(0);
        let days_remaining =
            ((expire_time - Utc::now().timestamp_millis()) as f64 / 86_400_000.0).ceil() as i64;
        let san = value
            .get("san")
            .and_then(Value::as_array)
            .map(|items| {
...
fn ensure_managed_domain_record(host: &str, tunnel_id: Option<&str>) -> Result<(), String> {
    ...
    let domain_id = DomainId::new(domain_id_for_host(host)).map_err(|error| error.to_string())?;
    ...
    if let Some(tunnel_id) = tunnel_id.filter(|value| !value.trim().is_empty()) {
        builder = builder.tunnel_id(TunnelId::new(tunnel_id).map_err(|error| error.to_string())?);
    }
