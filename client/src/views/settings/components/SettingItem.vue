...
const disabled = computed(() => ("disabled" in control.value && control.value.disabled === true) || props.item.readonly === true)
const canReset = computed(() => !props.item.readonly && control.value.type !== "action" && control.value.type !== "readonly")
...
const numberValue = computed(() => (typeof props.value === "number" ? props.value : Number(props.value ?? 0)))
...
const selectOptions = computed<SettingOption[]>(() => (control.value.type === "select" ? control.value.options : []))
const radioOptions = computed<SettingOption[]>(() => (control.value.type === "radio" ? control.value.options : []))
const colorSwatches = computed(() => (control.value.type === "color" ? control.value.swatches ?? [] : []))
const actions = computed<SettingAction[]>(() => (control.value.type === "action" ? control.value.actions : []))
const inputPlaceholder = computed(() => (control.value.type === "input" ? control.value.placeholder ?? "" : ""))
const inputType = computed(() => (control.value.type === "input" ? control.value.inputType ?? "text" : "text"))
const folderPlaceholder = computed(() => (control.value.type === "folder" ? control.value.placeholder ?? "选择文件夹" : "选择文件夹"))
const folderDisabled = computed(() => disabled.value || control.value.type === "folder")
const readonlyVariant = computed(() => (control.value.type === "readonly" ? control.value.variant ?? "text" : "text"))
