...
export function useSettingSearch() {
  ...
  const { categories, searchQuery, categoryFilter, activeCategoryId, selectedSettingId } =
    storeToRefs(store)
  ...
  const visibleCategories = computed(() => {
    ...
    const categoryId =
      categoryFilter.value === 'all' ? activeCategoryId.value : categoryFilter.value
  ...
  const isSearching = computed(
    () => searchQuery.value.trim().length > 0 || categoryFilter.value !== 'all',
  )
