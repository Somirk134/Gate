  ...
  <div class="server-projects">
    <div class="server-section__head">
      <div class="server-section__title">
        ...
        <span>Project 列表</span>
        <GBadge variant="neutral" type="soft" size="sm">{{ projects.length }}</GBadge>
      </div>
    </div>
    ...
    <div v-if="projects.length" class="server-sublist">
      ...
      >
        <span class="server-sublist__icon" :style="{ color: project.color, background: project.color + '1f' }">
          <GIcon name="package" :size="14" />
        </span>
        <div class="server-sublist__main">
          <span class="server-sublist__name">{{ project.name }}</span>
          <span class="server-sublist__meta">{{ project.tunnelCount }} Tunnels · {{ project.remark || "—" }}</span>
        </div>
    ...
    <div v-else class="server-logs__empty">
      ...
      <span>该服务器暂无 Project</span>
    </div>

    <p class="server-connection__hint">
      <GIcon name="info-circle" :size="12" />
      展示关联到当前服务器的 Project。全部 Mock 数据，未来从 Project 模块联动。
    </p>
  </div>
</template>
