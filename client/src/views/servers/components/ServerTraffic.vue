  ...
  <div class="server-traffic">
    ...
    <div class="server-traffic-grid">
      <div class="server-traffic-card">
        ...
        <span class="server-traffic-card__value">{{ formatSpeed(server.traffic.uploadSpeed) }}</span>
        <span class="server-traffic-card__sub">今日 {{ formatBytes(server.traffic.todayUpload) }}</span>
      </div>
      ...
      <div class="server-traffic-card">
        ...
        <span class="server-traffic-card__value">{{ formatSpeed(server.traffic.downloadSpeed) }}</span>
        <span class="server-traffic-card__sub">今日 {{ formatBytes(server.traffic.todayDownload) }}</span>
      </div>
      ...
      <div class="server-traffic-card">
        ...
        <span class="server-traffic-card__value">{{ formatBytes(server.traffic.totalUpload) }}</span>
        <span class="server-traffic-card__sub">峰值 {{ formatSpeed(server.statistics.peakSpeed) }}</span>
      </div>
      ...
      <div class="server-traffic-card">
        ...
        <span class="server-traffic-card__value">{{ formatBytes(server.traffic.totalDownload) }}</span>
    ...
    >
      ...
      <div class="server-conn-table">
        ...
        >
          ...
          <span class="server-conn-row__cell mono">{{ formatBytes(row.upload + row.download) }}</span>
  ...
  return [
    {
      ...
      percent: Math.round(((props.server.traffic.todayUpload + props.server.traffic.todayDownload) / total) * 100),
    },
    {
      label: "本周",
      upload: props.server.traffic.weekUpload,
      download: props.server.traffic.weekDownload,
      percent: Math.round(((props.server.traffic.weekUpload + props.server.traffic.weekDownload) / total) * 100),
    },
    {
      label: "本月",
      upload: props.server.traffic.monthUpload,
      download: props.server.traffic.monthDownload,
      percent: Math.round(((props.server.traffic.monthUpload + props.server.traffic.monthDownload) / total) * 100),
    },
    {
      label: "累计",
