...
export const useServerStore = defineStore("server-module", () => {
  ...
  const unhealthyServers = computed(() => servers.value.filter((s) => s.health.overall !== "healthy"))
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      servers.value = payload.items.map((row) => mapRuntimeServer(row, payload.activeServerId ?? null))
...
function mapRuntimeServer(row: RuntimeServerRecord, activeId: string | null): Server {
  ...
  const lastConnectedAt = row.lastConnectedAt ? formatRelative(row.lastConnectedAt) : "Never"
  ...
  return {
    ...
    region: row.region,
    publicIp: row.host,
    version: row.sessionId ? "connected" : "unknown",
    status,
    ...
    overview: {
      ...
      arch: "unknown",
      rustVersion: "unknown",
      serverVersion: row.sessionId ? "authenticated" : "unknown",
      installTime: createdAt,
      lastOnline: lastConnectedAt,
      lastHeartbeat: row.lastCheckedAt ? formatRelative(row.lastCheckedAt) : "Never",
    },
...
function healthFromStatus(status: ServerStatus, host: string, lastError: string | null, ping: number) {
    ...
    items: [
      {
        ...
        message: lastError ?? (healthy ? "Runtime authenticated" : `Waiting for connection to ${host}`),
        latency: ping,
