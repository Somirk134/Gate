<template>
  <header class="log-toolbar">
    ...
    <div class="log-toolbar__actions">
      <GIconButton name="arrow-down" :active="autoScroll" tooltip="Auto scroll" @click="$emit('toggle-auto-scroll')" />
      <GIconButton :name="paused ? 'play' : 'pause'" :active="paused" :tooltip="paused ? 'Resume' : 'Pause'" @click="togglePaused" />
      <GIconButton name="trash" tooltip="Clear logs" @click="$emit('clear')" />
      <GIconButton name="download" tooltip="Export logs" @click="$emit('export')" />
      <GIconButton name="copy" tooltip="Copy all" @click="$emit('copy-all')" />
      <GIconButton name="refresh" tooltip="Refresh" @click="$emit('refresh')" />
    </div>
  </header>
</template>
