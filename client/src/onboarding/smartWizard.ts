...
export const knowledgeCards: KnowledgeCard[] = [
  {
    ...
    title: "什么是 Tunnel？",
    icon: "router",
    body: "Tunnel 像一条临时通道，把别人从公网来的访问，送到你电脑里的本地服务。",
  },
  {
    id: "public-server",
    title: "什么是公网服务器？",
...
export const cloudProviders: CloudProviderOption[] = [
  { id: "tencent", name: "腾讯云", note: "国内访问稳定，适合微信、企微、国内支付回调。", tone: "mainland" },
  { id: "aliyun", name: "阿里云", note: "国内生态完整，适合生产环境和备案域名。", tone: "mainland" },
  ...
  { id: "digitalocean", name: "DigitalOcean", note: "文档友好，适合第一次使用云服务器。", tone: "global" },
  { id: "railway", name: "Railway", note: "体验部署友好，适合快速验证 Web 服务。", tone: "experience" },
  { id: "zeabur", name: "Zeabur", note: "适合轻量体验和应用托管，后续预留一键部署。", tone: "experience" },
]
...
export const scenarioRecommendations: ScenarioRecommendation[] = [
  ...
  },
  {
    ...
    certificate: "Not required",
    tags: ["SSH", "TCP"],
    reason: "SSH 不是网页协议，推荐 TCP Tunnel，并使用高位公网端口降低误扫风险。",
  },
  {
    id: "mysql",
    title: "MySQL",
  ...
  },
  {
    ...
    certificate: "Not required",
    tags: ["NAS"],
    reason: "NAS 常见管理页先用 HTTP/IP+Port 即可，正式外放再升级 HTTPS。",
  },
  {
    id: "minecraft",
    title: "Minecraft",
...
export function findScenario(id: string): ScenarioRecommendation {
  return scenarioRecommendations.find((scenario) => scenario.id === id) ?? scenarioRecommendations[0]
}
...
export function buildSmartRecommendation(answers: SmartWizardAnswers): SmartRecommendation {
  ...
  const remotePort = hasDomain && scenario.protocol !== "tcp" ? scenario.remotePort : scenario.remotePort
  const server =
  ...
  const certificate = protocol === "https" ? (hasDomain ? "Auto" : "绑定域名后自动申请") : "不需要"
  const tunnelName = answers.customName.trim() || scenario.defaultName
  const domainLabel = hasDomain ? domain : "暂不使用，先通过 IP + Port 访问"
  ...
  const deployMethod = environment?.recommendedDeploy ?? "选择服务器环境后，Gate 会给出推荐部署方式。"
  const domainReason = hasDomain
    ? `已使用 ${domain}，适合配置 HTTPS 和证书。`
    : "没有域名也可以使用 IP+Port；需要支付回调时建议补充域名。"
  const serverReason =
