...
<template>
  <div class="g-circle-progress" :style="{ width: `${size}px`, height: `${size}px` }">
    ...
    <span v-if="showLabel" class="g-circle-progress__label"> {{ Math.round(clamp(value)) }}% </span>
  </div>
</template>
...
const dashoffset = computed(() => circumference.value * (1 - clamp(props.value) / 100))
...
.g-circle-progress__bar--primary {
  stroke: var(--color-primary);
}
.g-circle-progress__bar--success {
  stroke: var(--color-success);
}
.g-circle-progress__bar--warning {
  stroke: var(--color-warning);
}
.g-circle-progress__bar--error {
